...
ngx_rtmp_edge_log(ngx_uint_t proType, ngx_uint_t logType, void *ss, ngx_uint_t current_ts, ...)
{
    ...
    if (ss == NULL || global_log == NULL)
        return;
    
    switch (logType) {
        case NGX_EDGE_PULL_START:
            szformat = "EDGE{\"_type\":\"v2.edgePullStart\",\"timestamp\":%l,\"session\":\"%s\",\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"%s\",\"responseTime\":%l,\"pullUrl\":\"%V\"}EDGE";
        ...
        case NGX_EDGE_PULL_WATCH:
            szformat = "EDGE{\"_type\":\"v2.edgePullWatch\",\"timestamp\":%l,\"session\":%s,\"clientIP\":%V,\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":%s,\"protocolType\":\"%s\",\"pullUrl\":\"%V\",\"pts\":%l,\"videoSize\":%l,\"audioSize\":%l,\"delay\":%l,\"sendFrame\":%l,\"dropVideoFrame\":%l,\"cacheVideoFrame\":%l}EDGE";
            if (proType == NGX_EDGE_RTMP) {
                s = (ngx_rtmp_session_t *)ss;
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, s->uuid, 
                        &s->client_ip, &s->server_ip, &s->host, &s->name, 
        ...
        case NGX_EDGE_PULL_STOP:
            szformat = "EDGE{\"_type\":\"v2.edgePullStop\",\"timestamp\":%l,\"session\":\"%s\",\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"%s\",\"pullUrl\":\"%V\",\"duration\":%l,\"statusCode\":%l,\"videoSize\":%l,\"audioSize\":%l,\"allDropFrame\":%l}EDGE";
            if (proType == NGX_EDGE_RTMP) {
                s = (ngx_rtmp_session_t *)ss;
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, s->uuid,
                        &s->client_ip, &s->server_ip, &s->host, &s->name, 
                        ngx_edge_type[proType], &s->pull_url, 0, 
                        s->status_code, s->recv_video_size, s->recv_audio_size, 0);
            } else if (proType == NGX_EDGE_HTTP) {
                pr = (ngx_http_live_play_request_ctx_t *)ss;
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, pr->uuid, 
                        &pr->client_ip, &pr->server_ip, &pr->host, &pr->stream, 
        ...
        case NGX_EDGE_PUSH_START:
            szformat = "EDGE{\"_type\":\"v2.edgePushStart\",\"timestamp\":%l,\"session\":%s,\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"rtmp\",\"firstRecvTime\":%l,\"url\":\"%V\",\"vFormat\":\"h264\",\"vFps\":%l,\"vBitRate\":%l,\"aFormat\":\"aac\",\"aChannel\":%l,\"aSamplerate\":%l}EDGE";
            if (proType == NGX_EDGE_RTMP) {
                s = (ngx_rtmp_session_t *)ss;
                codec = ngx_rtmp_get_module_ctx(s, ngx_rtmp_codec_module);
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, s->uuid, 
                        &s->client_ip, &s->server_ip, &s->host, &s->name, current_ts, &s->pull_url, 
        ...
        case NGX_EDGE_PUSH_WATCH:
            szformat = "EDGE{\"_type\":\"v2.edgePushWatch\",\"timestamp\":%l,\"session\":\"%s\",\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"rtmp\",\"recvVideoSize\":%l,\"recvAudioSize\":%l,\"recvVideoFrame\":%l,\"sendVideoSize\":%l,\"sendAudioSize\":%l}EDGE";
            if (proType == NGX_EDGE_RTMP) {
                s = (ngx_rtmp_session_t *)ss;
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, s->uuid, 
                        &s->client_ip, &s->server_ip, &s->host, &s->name, 
        ...
        case NGX_EDGE_PUSH_STOP:
            szformat = "EDGE{\"_type\":\"v2.edgePushStop\",\"timestamp\":%l,\"session\":\"%s\",\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"rtmp\",\"statusCode\":%l,\"recvVideoSize\":%l,\"recvAudioSize\":%l,\"recvVideoFrame\":%l,\"sendVideoSize\":%l,\"sendAudioSize\":%l}EDGE";
            if (proType == NGX_EDGE_RTMP) {
                s = (ngx_rtmp_session_t *)ss;
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, s->uuid, 
                        &s->client_ip, &s->server_ip, &s->host, &s->name, s->status_code, 
        ...
        case NGX_EDGE_BUFFER_START:
            szformat = "EDGE{\"_type\":\"v2.edgeBufferStart\",\"timestamp\":%l,\"session\":\"%s\",\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"%s\",\"cacheVideoFrame\":%l,\"cacheDuration\":%l}EDGE";
        ...
        case NGX_EDGE_BUFFER_STOP:
            szformat = "EDGE{\"_type\":\"v2.edgeBufferStop\",\"timestamp\":%l,\"session\":\"%s\",\"clientIP\":\"%V\",\"serverIP\":\"%V\",\"host\":\"%V\",\"name\":\"%V\",\"protocolType\":\"%s\",\"dropVideoSize\":%l,\"dorpAudioSize\":%l,\"dropVideoFrame\":%l,\"duration\":%l}EDGE";
            if (proType == NGX_EDGE_HTTP) {
                pr = (ngx_http_live_play_request_ctx_t *)ss;
                ngx_log_error(NGX_LOG_INFO, global_log, 0, szformat, current_ts, pr->uuid, 
                        &pr->client_ip, &pr->server_ip, &pr->host, &pr->stream, 
